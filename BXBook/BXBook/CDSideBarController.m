...
- (CDSideBarController*)initWithImages:(NSArray*)images
{
    ...
    _menuButton.frame = CGRectMake(0, 0, 80, 80);
    ...
    for (UIImage *image in [images copy])
    {
        ...
        button.frame = CGRectMake(20, 50 + (100 * index), 80, 80);
        button.tag = index;
...
{
    _menuButton.frame = CGRectMake(position.x+210, position.y, _menuButton.frame.size.width, _menuButton.frame.size.height);
    ...
    //    _backgroundMenuView.frame = CGRectMake(view.frame.size.width+256, 0, 120, view.frame.size.height);
    _backgroundMenuView.frame = CGRectMake(view.frame.size.width+5, 0, 120, view.frame.size.height);
...
- (void)performOpenAnimation
{
    dispatch_async(dispatch_get_main_queue(), ^{
        [UIView animateWithDuration:0.4 animations:^{
            ...
            _menuButton.transform = CGAffineTransformTranslate(CGAffineTransformIdentity, -120, 0);
            _backgroundMenuView.transform = CGAffineTransformTranslate(CGAffineTransformIdentity, -120, 0);
        }];
    });
    for (UIButton *button in _buttonList)
    {
